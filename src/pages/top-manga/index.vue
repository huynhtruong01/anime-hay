<template>
    <section class="pb-16" ref="el">
        <div class="h-[85vh]">
            <HeroBackground url="/images/top-manga-hero.png" />
        </div>
        <Container class="-mt-32">
            ...
            <div class="mt-20 grid grid-cols-5 gap-6">
                <div v-for="anime in animes" :key="anime.mal_id">
                    <AnimeCard :anime="anime" link="top-manga" />
                </div>
            </div>
...
const getAnimes = async () => {
    ...
    const { data }: any = await useFetch(`https://api.jikan.moe/v4/top/manga?page=${page.value + 1}`)
